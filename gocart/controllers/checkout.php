...
class Checkout extends Front_Controller {
	...
	function step_1()
	{
		...
		else
		{
			...
			$customer['ship_address']['company']=$customer['bill_address']['company']		= $this->input->post('company');
			$customer['ship_address']['firstname']=$customer['bill_address']['firstname']	= $this->input->post('firstname');
			$customer['ship_address']['lastname']=$customer['bill_address']['lastname']		= $this->input->post('lastname');
			$customer['ship_address']['email']=$customer['bill_address']['email']		= $this->input->post('email');
			$customer['ship_address']['phone']=$customer['bill_address']['phone']		= $this->input->post('phone');
			$customer['ship_address']['address1']=$customer['bill_address']['address1']		= $this->input->post('address1');
			$customer['ship_address']['address2']=$customer['bill_address']['address2']	= $this->input->post('address2');
			$customer['ship_address']['city']=$customer['bill_address']['city']			= $this->input->post('city');
			$customer['ship_address']['zip']=$customer['bill_address']['zip']			= $this->input->post('zip');
			...
			if($this->Location_model->has_zones($country->id))
			{
				...
				$customer['ship_address']['zone'] = $customer['bill_address']['zone']			= $zone->code;  /*  save the state for output formatted addresses */
			} else {
				$customer['ship_address']['zone'] = $customer['bill_address']['zone'] 		= '';
			}
			$customer['ship_address']['country'] = $customer['bill_address']['country']		= $country->name; /*  some shipping libraries require country name */
			$customer['ship_address']['country_code'] = $customer['bill_address']['country_code']   = $country->iso_code_2; /*  some shipping libraries require the code */ 
			$customer['ship_address']['zone_id'] = $customer['bill_address']['zone_id']		= $this->input->post('zone_id');  /*  use the zone id to populate address state field value */
			$customer['ship_address']['country_id'] = $customer['bill_address']['country_id']		= $this->input->post('country_id');
			$customer['ship_address']['shipprice']		=$customer['bill_address']['shipprice']		= $country->shipprice;
	...
	function shipping_address()
	{
		...
		else
		{
			...
			$customer['ship_address']['shipprice']		= $country->shipprice;
	...
	{
		$customer				= $this->go_cart->customer();			
		$json = '["Flat Rate",{"num":'.$customer['ship_address']['shipprice'].',"str":"USD '.$customer['ship_address']['shipprice'].'"}]';
		$this->go_cart->set_additional_detail('shipping_notes', "Shipping price as per shipping country.");

		/* parse out the shipping information */
		$shipping_method	= json_decode($json);
		$shipping_code		= md5($json);
		/* set shipping info */
