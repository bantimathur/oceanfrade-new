...
Class Product_model extends CI_Model
{
	...
	function get_products($category_id = false, $limit = false, $offset = false, $by=false, $sort=false)
	{
		...
		if ($category_id)
		{
			...
			return $result;
		} else if($category_id == 0) { // used for home page
			$this->db->select('category_products.*, products.*, LEAST(IFNULL(NULLIF(saleprice, 0), price), price) as sort_price', false)->from('category_products')->join('products', 'category_products.product_id=products.id');
	...
	function count_products($id)
	{
		if($id) {
			...
			return $this->db->select('product_id')->from('category_products')->join('products', 'category_products.product_id=products.id')->count_all_results();
		}
	}
