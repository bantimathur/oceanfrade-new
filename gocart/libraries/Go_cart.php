...
class go_cart {
	...
	public function get_country() {
		...
		$country = $this->CI->session->userdata('country');
		return 'India';
		if($country == "" ) {
			$path = FCPATH.APPPATH."packages/getcountry/phpip2country.class.php";
			include($path);
			$dbConfigArray = array(
				'host' => 'localhost', //example host name
				'port' => 3306, //3306 -default mysql port number
				'dbName' => 'guruji', //example db name
				'dbUserName' => 'root', //example user name
				'dbUserPassword' => '', //example user password
				'tableName' => 'ip_to_country', //example table name
			);
			
			$phpIp2Country = new phpIp2Country('117.229.116.29',$dbConfigArray);        
			//echo "<pre>";
	...
	public function set_dollar_price() {
		return;
		$usd_rate = $this->CI->session->userdata('usd_rate');	
		if($usd_rate == "") {		
			$this->CI->session->set_userdata('usd_rate',60);
			return;
			$from = "USD";
			$to ="INR";
			$amount = 1;
			...
			$usd_rate = trim(strip_tags($finalData[0]),'INR');
			$this->CI->session->set_userdata('usd_rate',$usd_rate);
