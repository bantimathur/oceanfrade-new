...
<div class="checkout-box">
	...
	<div class='order-summary' >
			...
			<?php }?>
				...
				<div class="tab-content">
					<?php foreach ($payment_methods as $method=>$info):?>
						<div style='width:49%; float:left;' id="payment-<?php echo $method;?>" class="tab-pane<?php echo ($selected == $method)?' active':'';?>">
							<?php echo form_open('checkout/step_3', 'id="form-'.$method.'"');?>
								<input type="hidden" name="module" value="<?php echo $method;?>" />
								<?php echo $info['form'];?>
								<input style='<?php if($method=='payumoney'){ echo "background:#6ca637;";} ?>' class="btn btn-block btn-large btn-primary" type="submit" value="<?php echo lang('form_continue');?> with <?=$info['name']==''?"PayUMoney":$info['name']?>"/>
							</form>
						</div>
					<?php endforeach;?>
	 ...
	 <style type="text/css">
	 #payment-payumoney{     border-left: 2px solid #eee;
    padding-left: 1%;}
	 </style>