$("document").ready(function() {
		...
		$("#reset-frame1").click(function() {
			$(".fradeitm").val('');
			var index_vaa =0;
			$(".skidrow").each(function(){				
				$("#total_volume_cbm_"+index_vaa).html('');
				$("#total_weight_lbs_"+index_vaa).html('');
				$("#total_vl_cbm").html('');
				$("#total_weight_cbm").html('');
				index_vaa++;
			});
		});
	$(".fradeitm").addClass('disabledbutton');
$("#calculate").addClass('disabledbutton');
$("#addmore").addClass('disabledbutton');
		...
		$("#bill_country").live("change",function() {			
         ...
         $(function() {
                $( "#shipped_on" ).datepicker({
                            showOn: "both",
						  buttonImage: "https://shipmyfreight.ca/images/calendar.gif",
        ...
        if (iKeyCode != 46 && iKeyCode > 31 && (iKeyCode < 48 || iKeyCode > 57))
			...
			if($('#dimension:checked').val() != "cm/kg") {
				...
				if(number > parseInt(restr)){					
					alert("Maximum value allowed for this field is : "+restr);
					event.preventDefault();
				}
    ...
    $('#addmore').live('keyup', function (evt) {       
     ...
     $("#addmore").click(function() {
          ...
          //cytab = parseInt(cytab) + 1;
		      ...
		      index = index + 1;
          var rel = $(this).attr('rel');
          var str = '<tr class="skidrow"  rel="'+index+'" id="rw_'+index+'"><td><input tabindex="'+cytab+'" type="text" name="iNoOfSkids[]" id="iNoOfSkids_'+index+'"  value="" maxlength="2" restr="10" restrkg="10" class="fradeitm lengthclass valid_id"></td>';
               cytab = parseInt(cytab) + 1;
              ...
              cytab = parseInt(cytab) + 1;
               str += '<td><input type="text" tabindex="'+cytab+'" name="fWeightPItem[]" id="fWeightPItem_'+index+'"  value="" restr="4600" restrkg="2086"  class="fradeitm"></td>';
                cytab = parseInt(cytab) + 1;
               str +=  '<td id="total_volume_cbm_'+index+'"></td>';                
                str +=  '<td id="total_weight_lbs_'+index+'"></td>';               
               str +=  '<td><img rel="'+index+'" src="https://shipmyfreight.ca/images/delete-icon.png" width="22" height="22" class="delete_row cursor"></td>';
               str +=   '</tr>';
                //alert(str);
               $("#wrapcont").before(str);			 
    ...
    $('#email_q').live('keyup', function (evt) {       
         ...
         if (!vld) {
            return false;
               ...
               $.post(url, { order_number: order_number ,sender_email : $('#sender_email').val()},
                    function(data) {                   
                        $('#sender_emailErr').html(' Email has been sent successfully, you will receive this quote shortly in your email. Please add our email info@shipmyfreight.ca in your address book to avoid delay or spam filter.');
                         setTimeout(function() {
     ...
     $("#vDestCity").live("change",function() {
		  ...
		  $(".fradeitm").removeClass('disabledbutton'); 
              $("#calculate").removeClass('disabledbutton');
$("#addmore").removeClass('disabledbutton');
